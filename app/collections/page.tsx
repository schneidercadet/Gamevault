...
export default function CollectionsPage() {
  ...
  const handleDeleteCollection = async (collectionId: string) => {
    try {
      ...
      if (success) {
        ...
        setError(new Error("Failed to delete collection"));
      }
    } catch (error) {
      setError(
  ...
  const handleRenameCollection = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!success) {
        setError(new Error("Failed to rename collection"));
      }
    } catch (error) {
      setError(
        error instanceof Error
  ...
  const handleRemoveGame = async (gameId: string) => {
    ...
    try {
      ...
      if (!success) {
        setError(new Error("Failed to remove game from collection"));
      }
    } catch (error) {
      setError(
        error instanceof Error
          ? error
          : new Error("Failed to remove game from collection")
      );
    }
  };
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {loading ? (
            ...
            sortedGames.map((game, index) => (
              <div key={`game-${game.id}-${index}`} className="group relative">
                <GameCard
                  game={{
                    ...game,
