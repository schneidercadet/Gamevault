  ...
  const handleAddOrRemoveClick = async (e: React.MouseEvent) => {
    ...
    if (isInCollection) {
      ...
      if (collections.length === 0) {
        try {
          console.log("Attempting to add game to 'all' collection", { 
            gameId: game.id,
          ...
          if (!success) {
            console.error("Failed to add game to collection - returned false");
            setError(new Error('Failed to add game to collection. Please try again.'));
          } else {
            console.log("Successfully added game to collection");
          }
        } catch (error) {
          console.error("Error adding game to collection:", error);
          setError(error instanceof Error ? error : new Error('Failed to add game to collection'));
        }
      } else {
